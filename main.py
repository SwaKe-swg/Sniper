    ...
    if pair_info and pair_info.get("chain") == "solana":
        ...
        if liquidity >= Config.MIN_LIQUIDITY and market_cap <= Config.MAX_MARKET_CAP and age_minutes <= Config.MAX_AGE_MINUTES:
            ...
            if token_address and token_address not in alerted_pairs_cache:
                print(f"[{datetime.now()}] Potenziale pair: {pair_info.get('baseToken', {}).get('symbol')}. Recupero info REST...")
                ...
                if rest_data and rest_data.get("pairs"):
                    ...
                    synthetic_pair_for_filter = {
                        ...
                        'age_minutes': (datetime.now().timestamp() - full_pair_data.get("pairCreatedAt", datetime.now().timestamp())) / 60 if full_pair_data.get("pairCreatedAt") else 0,
                        'holders': Config.MIN_HOLDERS + 1 # Assumi che abbia abbastanza holders se lo troviamo via REST
                    }
